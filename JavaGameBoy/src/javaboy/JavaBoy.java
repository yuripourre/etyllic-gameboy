...
public class JavaBoy extends Etyllica {
	...
	public static int[] keyCodes = {38, 40, 37, 39, 90, 88, 10, 8};
	...
	private void deactivateDebugger() {
	...
	public void hexDump(int address, int length) {
		...
		for (int l = 0; l < lines; l++) {
			System.out.print(hexWord(start + (l * 16)) + "   ");
			for (int r = start + (l * 16); r < start + (l * 16) + 16; r++) {
				System.out.print(hexByte(unsign(dmgcpu.addressRead(r))) + " ");
			}
			System.out.print("   ");
			for (int r = start + (l * 16); r < start + (l * 16) + 16; r++) {
	...
	public void showRegisterValues() {
		...
		System.out.print("A = " + hexWord(dmgcpu.a) + "    BC = " + hexWord(dmgcpu.b) + hexWord(dmgcpu.c));
		System.out.print("    DE = " + hexByte(dmgcpu.d) + hexByte(dmgcpu.e));
		System.out.print("    HL = " + hexWord(dmgcpu.hl));
		System.out.print("    PC = " + hexWord(dmgcpu.pc));
		System.out.println("    SP = " + hexWord(dmgcpu.sp));
		System.out.println("F = " + hexByte(unsign((short) dmgcpu.f)));
	}
		...
		try {
			...
			case 'd' :
				try {
					...
					System.out.println("- Dumping " + hexWord(length) + " instructions starting from " + hexWord(address));
			...
			case 'i' :
				try {
					int address = Integer.valueOf(st.nextToken(), 16).intValue();
					int length = Integer.valueOf(st.nextToken(), 16).intValue();
					System.out.println("- Dissasembling " + hexWord(length) + " instructions starting from " + hexWord(address));
			...
			case 'p' :
				try {
					int length = Integer.valueOf(st.nextToken(), 16).intValue();
					System.out.println("- Dissasembling " + hexWord(length) + " instructions starting from program counter (" + hexWord(dmgcpu.pc) + ")");
				...
				try {
					...
					try {
						...
						if (dmgcpu.setRegister(reg, val)) {
							System.out.println("- Set register " + reg + " to " + hexWord(val) + ".");
						} else {
			...
			case 'e' :
				...
				System.out.print("- Written data starting at " + hexWord(address) + " (");
				if (!st.hasMoreTokens()) {
					System.out.println("");
				...
				try {
					while (st.hasMoreTokens()) {
						...
						//           System.out.print(hexByte(unsign(data)));
			...
			case 'b' :
				try {
					...
					System.out.println("- Setting breakpoint at " + hexWord(addr));
			...
			case 'm' :
				try {
					...
					System.out.println("- Mapping ROM bank " + hexByte(bank) + " to 4000 - 7FFFF");
					cartridge.saveMapping();
			...
			case 't' :
				try {
					cartridge.restoreMapping();
					int length = Integer.valueOf(st.nextToken(), 16).intValue();
					System.out.println("- Executing " + hexWord(length) + " instructions starting from program counter (" + hexWord(dmgcpu.pc) + ")");
					execute(length);
					...
					System.out.println("- Executing instruction at program counter (" + hexWord(dmgcpu.pc) + ")");
					execute(1);
				} catch (NumberFormatException e) {
					System.out.println("Error parsing hex value.");
				}
