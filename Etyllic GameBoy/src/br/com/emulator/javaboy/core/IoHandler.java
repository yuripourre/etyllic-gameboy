...
public class IoHandler extends LowLevelData{
	...
	public void performHdma() {
		int dmaSrc = (unsign(registers[0x51]) << 8) +
		(unsign(registers[0x52]) & 0xF0);
		int dmaDst = ((unsign(registers[0x53]) & 0x1F) << 8) +
		(unsign(registers[0x54]) & 0xF0) + 0x8000;
	...
	public void ioWrite(int num, short data) {
     ...
     if ((registers[0x02] & 0x01) == 1) {
	  //System.out.println("Sent byte: " + JavaBoy.hexByte(unsign(registers[0x01])));
      registers[0x01] = (byte) 0xFF; // when no LAN connection
      dmgcpu.triggerInterrupt(dmgcpu.INT_SER);
      registers[0x02] &= 0x7F;
     }
    } else if (((unsign(data) & 0x81) == 0x81) && (dmgcpu.gameLink != null)) {
     dmgcpu.gameLink.send(registers[0x01]);
    }
//    System.out.println(JavaBoy.hexWord(dmgcpu.pc));*/
		...
		case 0x41 :
			//    System.out.println("STAT set to " + data + " lcdc is " + unsign(registers[0x44]) + " pc is " + JavaBoy.hexWord(dmgcpu.pc));
			registers[0x41] = (byte) data;
			break;

		case 0x42 :           // SCY
			//    System.out.println("SCY set to " + data + " lcdc is " + unsign(registers[0x44]) + " pc is " + JavaBoy.hexWord(dmgcpu.pc));
			registers[0x42] = (byte) data;
			break;

		case 0x43 :           // SCX
			//    System.out.println("SCX set to " + data + " lcdc is " + unsign(registers[0x44]) + " pc is " + JavaBoy.hexWord(dmgcpu.pc));
		...
		case 0x55 :
			if ((!hdmaRunning) && ((registers[0x55] & 0x80) == 0) && ((data & 0x80) == 0) ) {
				...
				int dmaDst = ((unsign(registers[0x53]) & 0x1F) << 8) +
				(unsign(registers[0x54]) & 0xF0) + 0x8000;
				int dmaLen = ((unsign(data) & 0x7F) * 16) + 16;
				...
				if ((unsign(data) & 0x80) == 0x80) {
					hdmaRunning = true;
					//      System.out.println("HDMA started");
					registers[0x55] = (byte) (data & 0x7F);
					break;
				} else if ((hdmaRunning) && ((unsign(data) & 0x80) == 0)) {
					hdmaRunning = false;
		...
		case 0x69 :           // FF69 - BCPD: GBC BG Palette data write
			...
			if (dmgcpu.isGbcFeatures()) {
				...
				dmgcpu.graphicsChip.gbcBackground[palNumber].setGbcColours(
						(unsign(registers[0x68]) & 0x06) >> 1,
						(unsign(registers[0x68]) & 0x01) == 1, unsign(data));
				dmgcpu.graphicsChip.invalidateAll(palNumber * 4);

				if ((unsign(registers[0x68]) & 0x80) != 0) {
					registers[0x68]++;
				}
		...
		case 0x6B :           // FF6B - OCPD: GBC Sprite Palette data write
			...
			dmgcpu.graphicsChip.gbcSprite[palNumber].setGbcColours(
					(unsign(registers[0x6A]) & 0x06) >> 1,
					(unsign(registers[0x6A]) & 0x01) == 1, unsign(data));
			dmgcpu.graphicsChip.invalidateAll((palNumber * 4) + 32);

			if ((unsign(registers[0x6A]) & 0x80) != 0) {
