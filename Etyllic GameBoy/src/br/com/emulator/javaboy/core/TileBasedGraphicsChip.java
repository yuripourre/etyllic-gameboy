...
public class TileBasedGraphicsChip extends GraphicsChip {
	...
	public void drawSprites(Graphics back, int priority) {
		...
		// Draw sprites
		for (int i = tileBankStart; i < 40; i++) {
	...
	public boolean draw(int startX, int startY) {
		int tileNum;
  ...
  for (int y = 0; y < 19; y++) {
   for (int x = 0; x < 21; x++) {
    ...
    tileAddress = bgStartAddress +
       (((y + yTileOfs) % 32) * 32) + ((x + xTileOfs) % 32);
    attributeData = unsign(videoRam[tileAddress + 0x2000]);
   ...
   g.drawImage(backBuffer, startX, startY, width, height, null);
	...
	class GameboyTile {
		...
		public void allocateImage(int attribs) {
			...
			image[attribs] = Toolkit.getDefaultToolkit().createImage(source[attribs]);
		}
