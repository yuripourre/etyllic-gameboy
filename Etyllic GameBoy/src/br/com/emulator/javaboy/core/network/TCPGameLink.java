...
public class TCPGameLink extends GameLink implements Runnable {
	private final int PORTNO = 1989;
	...
	public void run() {
		...
		while (((serverRunning) || (clientConnected)) && (!terminate)) {
			...
			try {
				...
				int data = 0, initial, clock, b1, b2, b3, b4, v = 0;
				while ((data != -1) && (!terminate)) {           /* This needs to terminate */

					if (dmgcpu != null) {
						v = dmgcpu.getInstrCount();
					}
					...
					if (dmgcpu != null) {
						while ( /*(v + 600 > dmgcpu.instrCount) || /*((clock != -1) && (clock > dmgcpu.instrCount)) ||*/ (!dmgcpu.interruptsEnabled) || ((dmgcpu.ioHandler.getRegisters()[0x0F] & dmgcpu.INT_SER) != 0) && (!terminate)) {
							try {
					...
					if (initial == 1) {
						...
						outStream.write(dmgcpu.ioHandler.getRegisters()[0x01]);
						outStream.flush();
						dmgcpu.ioHandler.getRegisters()[0x02] &= 0x7F;
						...
						dmgcpu.ioHandler.getRegisters()[0x01] = (byte) data;
	...
	public void send(byte b) {
		try {
			outStream.write(1);
			writeInt(outStream, dmgcpu.getInstrCount());
			outStream.write(b);
			outStream.flush();
